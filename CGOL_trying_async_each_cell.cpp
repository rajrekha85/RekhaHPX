...
vector<vector<int> > tryingToParallelize(int i , int j)
{	
	...
	*/
	...
	for(int i=1;i<=sizeofGrid-1;i++){
		for(int j=1;j<=sizeofGrid-1;j++){   
			...
			neighborCount=0;
			bufferGrid_Copy.push_back(async(launch::async, tryingToParallelize, i , j)); 
			vector<vector<int> > checkNew = bufferGrid_Copy[p].get();
			buffergrid[i][j]=checkNew[i][j];
			neighborCount=0;
			p = p+1;
		}
	}
